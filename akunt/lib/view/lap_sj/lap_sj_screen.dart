...
import '../../controller/laporan_sj_controller.dart';
import '../../controller/surat_controller.dart';
import 'widget/filter_tanggal.dart';

class LapSjScreen extends StatefulWidget {
  @override
  _LapSjScreenState createState() => _LapSjScreenState();
}

class _LapSjScreenState extends State<LapSjScreen> {
  String baseUrl = base_url;
  @override
  void initState() {
    Provider.of<LapSjController>(context, listen: false).initData();
    super.initState();
  }

  @override
  Widget build(BuildContext context) {
    return Consumer<LapSjController>(builder: (context, lapSj, child) {
      return Scaffold(
        appBar: AppBar(
        ...
        body: Container(
          ...
          child: Column(
            children: [
              ...
              Expanded(
                child: (lapSj.data_list.length > 0)
                    ? ListView(children: [
                        Padding(
                          padding: EdgeInsets.only(
                              left: 24, right: 24, top: 16, bottom: 4),
                          child: PaginatedDataTable(
                            source: MyData(lapSj.data_list, context, lapSj),
              ...
              children: [
                ...
                RichText(
                  text: TextSpan(
                    ...
                    children: [
                      TextSpan(
                        text: lapSj.data_list.length.toString(),
                        style: GoogleFonts.poppins(
                            fontWeight: FontWeight.w500,
                            fontSize: 16,
                            color: Colors.black),
  ...
  Widget pageField() {
    SuratController pageTerima =
        Provider.of<SuratController>(context, listen: false);
    return Container(
      width: 70,
      height: 35,
      child: TextField(
...
class MyData extends DataTableSource {
  ...
  LapSjController lapSj;
  MyData(this.dtx, this.context, this.lapSj);
