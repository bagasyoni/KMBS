...
import '../../controller/laporan_po_controller.dart';
import '../../model/data_supp.dart';
import 'widget/filter_tanggal.dart';

class IndexLSPScreen extends StatefulWidget {
  @override
  _IndexLSPScreenState createState() => _IndexLSPScreenState();
}

class _IndexLSPScreenState extends State<IndexLSPScreen> {
  String baseUrl = base_url;
  @override
  void initState() {
    Provider.of<LapPoController>(context, listen: false).initData();
    super.initState();
  }

  @override
  Widget build(BuildContext context) {
    return Consumer<LapPoController>(builder: (context, lapSp, child) {
      return Scaffold(
        appBar: AppBar(
          ...
          actions: [
            ...
            OnHoverButton(
              child: InkWell(
                hoverColor: Colors.white,
                onTap: () {
                  lapSp.proses_export_perSupplier(1);
                },
                child: Container(
                  height: 30,
                  child: Row(
        ...
        body: Container(
          ...
          child: Column(
            children: [
              ...
              Expanded(
                child: (lapSp.data_list.length > 0)
                    ? ListView(children: [
                        Padding(
                          padding: EdgeInsets.only(
                              left: 24, right: 24, top: 16, bottom: 4),
                          child: PaginatedDataTable(
                            source: MyData(lapSp.data_list, context, lapSp),
...
class MyData extends DataTableSource {
  ...
  LapPoController lapSp;
  MyData(this.dtx, this.context, this.lapSp);
