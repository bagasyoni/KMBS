...
import '../config/export_pdf_array_5.dart';
import '../model/model_lapmemo.dart';

class LapMemoController with ChangeNotifier {
  model_lapmemo m_lapmemo = model_lapmemo();
  ...
  Future<void> select_data() async {
    data_list = await m_lapmemo.data_lap(tanggal_awal, tanggal_akhir);
    notifyListeners();
  }
  ...
  Future<void> proses_export_lapmemo(int mode) async {
    if (data_list.length > 0) {
      ...
      header_excel.add("Debet");
      header_excel.add("Kredit");
      for (int i = 0; i < data_list.length; i++) {
        ...
        isi_map['d'] = data_list[i]['DEBET'];
        isi_map['e'] = data_list[i]['KREDIT'];
        isi_excel.add(isi_map);
      }
      String judul = "Laporan Memorial";
      String header_title = "";
      if (mode == 0) {
