...
class PobarangimportController with ChangeNotifier {
  final Future<SharedPreferences> _prefs = SharedPreferences.getInstance();
  SharedPreferences prefs;
  model_pobarangimport m_pobarang = model_pobarangimport();
  ...
  Future<void> selectDataPaginate(bool reload) async {
    ...
    data_pobarang_list = await m_pobarang.data_pobarangimportpaginate(
        searchController.text, offset, limit);
    home_pobarang_list = await m_pobarang.data_pobarangimportpaginate(
        searchController.text, offset, limit);
    var count =
        await m_pobarang.countPobarangimportPaginate(searchController.text);
  ...
  void modalData(String cari) async {
    data_pobarang_list = await model_pobarangimport().data_modal(cari);
    home_pobarang_list = await model_pobarangimport().data_modal(cari);
    notifyListeners();
  }
  ...
  Future<void> select_data() async {
    data_pobarang_list = await m_pobarang.select_po_barang_import(
        searchController.text, tanggal_awal, tanggal_akhir, perx);
    home_pobarang_list = await m_pobarang.select_po_barang_import(
        searchController.text, tanggal_awal, tanggal_akhir, perx);
    total = 0;
    qty = 0;
  ...
  void selectData(String cari) async {
    data_pobarang_list =
        await model_pobarangimport().cari_po_barang_import(cari);
    home_pobarang_list =
        await model_pobarangimport().cari_po_barang_import(cari);
  ...
  void proses_export() {
    if (data_pobarang_list.length > 0) {
      ...
      for (int i = 0; i < data_pobarang_list.length; i++) {
        ...
        isi_map['a'] = data_pobarang_list[i]['tanggal'];
        isi_map['b'] = data_pobarang_list[i]['no_bukti'];
        isi_map['c'] = data_pobarang_list[i]['sales'];
        isi_map['d'] = data_pobarang_list[i]['customer'];
        isi_map['e'] = data_pobarang_list[i]['keterangan'];
        isi_map['f'] = data_pobarang_list[i]['total_qty'];
        isi_map['g'] = data_pobarang_list[i]['total_so'];
        if (data_pobarang_list[i]['status'] == 1) {
          isi_map['h'] = "Diterima";
        } else {
  ...
  Future<void> proses_export_detail() async {
    if (index_terpilih != null) {
      ...
      map_transaksi['a'] = data_pobarang_list[index_terpilih]['tanggal'];
      map_transaksi['b'] = data_pobarang_list[index_terpilih]['no_bukti'];
      map_transaksi['c'] = data_pobarang_list[index_terpilih]['sales'];
      map_transaksi['d'] = data_pobarang_list[index_terpilih]['customer'];
      map_transaksi['e'] = data_pobarang_list[index_terpilih]['keterangan'];
      if (data_pobarang_list[index_terpilih]['status'] == 1) {
      ...
      List data_account = await m_pobarang.select_po_barang_import_detail(
          data_pobarang_list[index_terpilih]['no_bukti'], "NO_BUKTI", "pod");
      ...
      footer_excel.add(data_pobarang_list[index_terpilih]['total_qty']);
      footer_excel.add("Total");
      footer_excel.add(data_pobarang_list[index_terpilih]['total_so']);
      String judul =
          "Invoice Order Penjualan (${data_pobarang_list[index_terpilih]['no_bukti']})";
  ...
  Future<void> proses_print() async {
    List data_account = await m_pobarang.select_po_barang_import_detail(
        data_pobarang_list[index_terpilih]['no_bukti'], "NO_BUKTI", "po");
    InvoiceOrderPenjualan()
        .proses_print(data_pobarang_list[index_terpilih], data_account);
  }
  ...
  Future<void> initData_addPobarang() async {
    ...
    sumTotal = 0;
    await baca_periodePrefs();
    await m_pobarang.get_no_bukti('PO/NON/I', 'NO_BUKTI', 'po').then((value) {
      if (value != null) {
  ...
  Future<void> initData_editPobarang(var data_edit) async {
    ...
    tglController.text = format_tanggal.format(chooseDate);
    jtempoController.text = format_jtempo.format(chooseDateJT);
    List data_lama = await m_pobarang.select_po_barang_import_detail(
        ...
        if (no_buktiController.text.isNotEmpty) {
          if (data_brg_keranjang.length > 0) {
            ...
            if (data_ready.length > 0) {
              ...
              await m_pobarang.insert_po_barang_import(obj);
              BotToast.closeAllLoading();
              return true;
            }
          } else {
      ...
      if (periodeTgl == perx) {
        if (no_buktiController.text.isNotEmpty) {
          if (data_brg_keranjang.length > 0) {
            ...
            obj['ACNO1_NM'] = acno1_nmController.text;
            obj['tabeld'] = await baca_tabeld();
            await m_pobarang.update_po_barang_import(obj);
            BotToast.closeAllLoading();
  ...
  Future<bool> deletePobarang(String no_bukti) async {
    try {
      var delete = await m_pobarang.delete_po_barang_import(no_bukti);
      await select_data();
      return true;
    } catch (e) {
      return false;
