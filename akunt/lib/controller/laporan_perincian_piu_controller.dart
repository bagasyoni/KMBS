...
import '../model/model_lapperincian_piu.dart';
import '../model/model_periode.dart';

class LapPerincianpiuController with ChangeNotifier {
  final Future<SharedPreferences> _prefs = SharedPreferences.getInstance();
  SharedPreferences prefs;
  model_lapperincian_piu m_lapperincian_piu = model_lapperincian_piu();
  ...
  Future<void> select_data() async {
    ...
    for (int i = 0; i < data_list.length; i++) {
      ...
      jual += double.parse(data_list[i]['MASUK'].toString());
  ...
  Future<void> proses_export_lapperincianpiu(int mode) async {
    if (data_list.length > 0) {
      ...
      String judul = "Laporan Perincian Piutang";
      String header_title = "";
      if (mode == 0) {
