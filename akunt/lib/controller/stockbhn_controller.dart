...
import 'package:akunt/model/model_stockbhn.dart';
...
class StockbhnController with ChangeNotifier {
  ...
  Future<void> selectDataPaginate(bool reload) async {
    ...
    data_stockbhn_list = await m_stockbhn.data_stockbhnpaginate(
        searchController.text, offset, limit);
    home_stockbhn_list = await m_stockbhn.data_stockbhnpaginate(
        searchController.text, offset, limit);
    var count = await m_stockbhn.countStockbhnPaginate(searchController.text);
  ...
  void modalData(String cari) async {
    data_stockbhn_list = await model_stockbhn().data_modal(cari);
    home_stockbhn_list = await model_stockbhn().data_modal(cari);
    notifyListeners();
  }
  ...
  Future<void> select_data() async {
    data_stockbhn_list = await m_stockbhn.select_stockbhn(
        searchController.text, tanggal_awal, tanggal_akhir, perx);
    home_stockbhn_list = await m_stockbhn.select_stockbhn(
        searchController.text, tanggal_awal, tanggal_akhir, perx);
    total = 0;
    qty = 0;
    for (int i = 0; i < data_stockbhn_list.length; i++) {
      total += double.parse(data_stockbhn_list[i]['TOTAL'].toString());
      qty += double.parse(data_stockbhn_list[i]['total_qty'].toString());
    }
    notifyListeners();
  }

  void selectData(String cari) async {
    data_stockbhn_list = await model_stockbhn().cari_stock_bhn(cari);
    home_stockbhn_list = await model_stockbhn().cari_stock_bhn(cari);
  ...
  void proses_export() {
    if (data_stockbhn_list.length > 0) {
      ...
      for (int i = 0; i < data_stockbhn_list.length; i++) {
        ...
        isi_map['a'] = data_stockbhn_list[i]['tanggal'];
        isi_map['b'] = data_stockbhn_list[i]['no_bukti'];
        isi_map['c'] = data_stockbhn_list[i]['sales'];
        isi_map['d'] = data_stockbhn_list[i]['customer'];
        isi_map['e'] = data_stockbhn_list[i]['keterangan'];
        isi_map['f'] = data_stockbhn_list[i]['total_qty'];
        isi_map['g'] = data_stockbhn_list[i]['total_so'];
        if (data_stockbhn_list[i]['status'] == 1) {
          isi_map['h'] = "Diterima";
        } else {
  ...
  Future<void> proses_export_detail() async {
    if (index_terpilih != null) {
      ...
      map_transaksi['a'] = data_stockbhn_list[index_terpilih]['tanggal'];
      map_transaksi['b'] = data_stockbhn_list[index_terpilih]['no_bukti'];
      map_transaksi['c'] = data_stockbhn_list[index_terpilih]['sales'];
      map_transaksi['d'] = data_stockbhn_list[index_terpilih]['customer'];
      map_transaksi['e'] = data_stockbhn_list[index_terpilih]['keterangan'];
      if (data_stockbhn_list[index_terpilih]['status'] == 1) {
      ...
      List data_bhn = await m_stockbhn.select_stockbhn_detail(
          data_stockbhn_list[index_terpilih]['no_bukti'], "NO_BUKTI", "kas");
      ...
      footer_excel.add(data_stockbhn_list[index_terpilih]['total_qty']);
      footer_excel.add("Total");
      footer_excel.add(data_stockbhn_list[index_terpilih]['total_so']);
      String judul =
          "Invoice Order Penjualan (${data_stockbhn_list[index_terpilih]['no_bukti']})";
  ...
  Future<void> proses_print() async {
    List data_bhn = await m_stockbhn.select_stockbhn_detail(
        data_stockbhn_list[index_terpilih]['no_bukti'], "no_bukti", "stocka");
    InvoiceOrderPenjualan()
        .proses_print(data_stockbhn_list[index_terpilih], data_bhn);
  }
  ...
  Future<void> initData_addStockbhn() async {
    ...
    await baca_periodePrefs();
    await m_stockbhn
        .get_no_bukti('KA/NON/I', 'NO_BUKTI', 'stocka')
        .then((value) {
      if (value != null) {
        no_buktiController.text =
            "KA/NON/I/${format_no_bukti.format(DateTime.now())}/${value[0]['NOMOR']}";
      }
    });
  ...
  ///HEADER///
  Future<void> initData_editStockbhn(var data_edit) async {
    ...
    tglController.text = format_tanggal.format(chooseDate);
    List data_lama = await m_stockbhn.select_stockbhn_detail(
  ...
  /// data header
  Future<bool> saveStockbhn() async {
    hitungSubTotal();
    if (no_buktiController.text.isNotEmpty) {
      if (data_bhn_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_stockbhn.check_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_stockbhn.insert_stockbhn(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editStockbhn() async {
    hitungSubTotal();
    if (no_buktiController.text.isNotEmpty) {
      if (data_bhn_keranjang.length > 0) {
        BotToast.showLoading();
        ...
        obj['tabeld'] = await baca_tabeld();
        await m_stockbhn.update_stockbhn(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteStockbhn(String no_bukti) async {
    try {
      var delete = await m_stockbhn.delete_stock_bhn(no_bukti);
      await select_data();
      return true;
    } catch (e) {
      return false;
