...
import 'package:akunt/model/model_pononbahan.dart';
import 'package:akunt/view/base_widget/toast.dart';
import 'package:syncfusion_flutter_datepicker/datepicker.dart';

class PononbahanController with ChangeNotifier {
  model_pononbahan m_pononbahan = model_pononbahan();
  TextEditingController searchController = TextEditingController();
  DateRangePickerController filter_tanggalController =
      new DateRangePickerController();
  List data_pononbahan_list = [];
  ...
  void modalData(String cari) async {
    data_pononbahan_list = await model_pononbahan().data_modal(cari);
    notifyListeners();
  }

  Future<void> select_data() async {
    data_pononbahan_list = await m_pononbahan.select_po_nonbahan(
        searchController.text, tanggal_awal, tanggal_akhir);
    notifyListeners();
  }

  void selectData(String cari) async {
    data_pononbahan_list = await model_pononbahan().cari_po_nonbahan(cari);
    notifyListeners();
  }
  ...
  void proses_export() {
    if (data_pononbahan_list.length > 0) {
      ...
      for (int i = 0; i < data_pononbahan_list.length; i++) {
        Map<String, dynamic> isi_map = new Map<String, dynamic>();
        isi_map['a'] = data_pononbahan_list[i]['tanggal'];
        isi_map['b'] = data_pononbahan_list[i]['no_bukti'];
        isi_map['c'] = data_pononbahan_list[i]['sales'];
        isi_map['d'] = data_pononbahan_list[i]['customer'];
        isi_map['e'] = data_pononbahan_list[i]['keterangan'];
        isi_map['f'] = data_pononbahan_list[i]['total_qty'];
        isi_map['g'] = data_pononbahan_list[i]['total_so'];
        if (data_pononbahan_list[i]['status'] == 1) {
          isi_map['h'] = "Diterima";
        } else {
  ...
  Future<void> proses_export_detail() async {
    if (index_terpilih != null) {
      ...
      map_transaksi['a'] = data_pononbahan_list[index_terpilih]['tanggal'];
      map_transaksi['b'] = data_pononbahan_list[index_terpilih]['no_bukti'];
      map_transaksi['c'] = data_pononbahan_list[index_terpilih]['sales'];
      map_transaksi['d'] = data_pononbahan_list[index_terpilih]['customer'];
      map_transaksi['e'] = data_pononbahan_list[index_terpilih]['keterangan'];
      if (data_pononbahan_list[index_terpilih]['status'] == 1) {
      ...
      List data_account = await m_pononbahan.select_po_nonbahan_detail(
          data_pononbahan_list[index_terpilih]['no_bukti'], "NO_BUKTI", "kas");
      ...
      footer_excel.add(data_pononbahan_list[index_terpilih]['total_qty']);
      footer_excel.add("Total");
      footer_excel.add(data_pononbahan_list[index_terpilih]['total_so']);
      String judul =
          "Invoice Order Penjualan (${data_pononbahan_list[index_terpilih]['no_bukti']})";
  ...
  Future<void> proses_print() async {
    List data_account = await m_pononbahan.select_po_nonbahan_detail(
        data_pononbahan_list[index_terpilih]['no_bukti'], "NO_BUKTI", "po");
    InvoiceOrderPenjualan()
        .proses_print(data_pononbahan_list[index_terpilih], data_account);
  }
  ...
  Future<void> initData_addPononbahan() async {
    ...
    await m_pononbahan
      ...
      if (value != null) {
        ...
        nobuktiController.text =
            "PO${format_no_bukti.format(DateTime.now())}NB-${no_urut + 1}";
      }
    });
  ...
  ///HEADER///
  Future<void> initData_editPononbahan(var data_edit) async {
    ...
    List data_lama = await m_pononbahan.select_po_nonbahan_detail(
  ...
  /// data header
  Future<bool> savePononbahan() async {
    hitungSubTotal();
    if (nobuktiController.text.isNotEmpty) {
      if (data_brg_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_pononbahan.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_pononbahan.insert_po_nonbahan(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editPononbahan() async {
    hitungSubTotal();
    if (nobuktiController.text.isNotEmpty) {
      if (data_brg_keranjang.length > 0) {
        BotToast.showLoading();
        ...
        obj['total'] = sumTotal;
        obj['tabeld'] = await baca_tabeld();
        await m_pononbahan.update_po_nonbahan(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deletePononbahan(String no_bukti) async {
    try {
      var delete = await m_pononbahan.delete_po_nonbahan(no_bukti);
      await select_data();
      return true;
    } catch (e) {
      return false;
