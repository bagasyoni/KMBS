...
import '../model/model_lapkartustk_brg.dart';
import '../model/model_periode.dart';

class LapKartustkbrgController with ChangeNotifier {
  final Future<SharedPreferences> _prefs = SharedPreferences.getInstance();
  SharedPreferences prefs;
  model_lapkartustk_brg m_lapkartustk_brg = model_lapkartustk_brg();
  ...
  Future<void> select_data() async {
    ...
    data_list = await m_lapkartustk_brg.data_lap(
  ...
  Future<void> proses_export_lapkartustkbrg(int mode) async {
    if (data_list.length > 0) {
      ...
      for (int i = 0; i < data_list.length; i++) {
        ...
        isi_map['i'] = data_list[i]['LAIN'];
        isi_map['j'] = data_list[i]['SALDO'];
        isi_map['t_aw'] = awal;
        isi_map['t_ma'] = masuk;
        isi_map['t_ke'] = keluar;
        isi_map['t_ln'] = lain;
      ...
      String judul = "Laporan Kartu Stok Barang";
      String header_title = "";
      if (mode == 0) {
