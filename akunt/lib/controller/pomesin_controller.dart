...
import 'package:akunt/model/model_pomesin.dart';
import 'package:akunt/view/base_widget/toast.dart';
import 'package:syncfusion_flutter_datepicker/datepicker.dart';

class PomesinController with ChangeNotifier {
  model_pomesin m_pomesin = model_pomesin();
  TextEditingController searchController = TextEditingController();
  DateRangePickerController filter_tanggalController =
      new DateRangePickerController();
  List data_pomesin_list = [];
  ...
  void modalData(String cari) async {
    data_pomesin_list = await model_pomesin().data_modal(cari);
    notifyListeners();
  }

  Future<void> select_data() async {
    data_pomesin_list = await m_pomesin.select_po_mesin(
        searchController.text, tanggal_awal, tanggal_akhir);
    notifyListeners();
  }

  void selectData(String cari) async {
    data_pomesin_list = await model_pomesin().cari_po_mesin(cari);
    notifyListeners();
  }
  ...
  void proses_export() {
    if (data_pomesin_list.length > 0) {
      ...
      for (int i = 0; i < data_pomesin_list.length; i++) {
        Map<String, dynamic> isi_map = new Map<String, dynamic>();
        isi_map['a'] = data_pomesin_list[i]['tanggal'];
        isi_map['b'] = data_pomesin_list[i]['no_bukti'];
        isi_map['c'] = data_pomesin_list[i]['sales'];
        isi_map['d'] = data_pomesin_list[i]['customer'];
        isi_map['e'] = data_pomesin_list[i]['keterangan'];
        isi_map['f'] = data_pomesin_list[i]['total_qty'];
        isi_map['g'] = data_pomesin_list[i]['total_so'];
        if (data_pomesin_list[i]['status'] == 1) {
          isi_map['h'] = "Diterima";
        } else {
  ...
  Future<void> proses_export_detail() async {
    if (index_terpilih != null) {
      ...
      map_transaksi['a'] = data_pomesin_list[index_terpilih]['tanggal'];
      map_transaksi['b'] = data_pomesin_list[index_terpilih]['no_bukti'];
      map_transaksi['c'] = data_pomesin_list[index_terpilih]['sales'];
      map_transaksi['d'] = data_pomesin_list[index_terpilih]['customer'];
      map_transaksi['e'] = data_pomesin_list[index_terpilih]['keterangan'];
      if (data_pomesin_list[index_terpilih]['status'] == 1) {
      ...
      List data_mesin = await m_pomesin.select_po_mesin_detail(
          data_pomesin_list[index_terpilih]['no_bukti'], "NO_BUKTI", "kas");
      for (int i = 0; i < data_mesin.length; i++) {
        Map<String, dynamic> isi_map = new Map<String, dynamic>();
        isi_map['a'] = data_mesin[i]['kd_brg'];
        isi_map['b'] = data_mesin[i]['na_brg'];
        isi_map['c'] = data_mesin[i]['satuan'];
        isi_map['d'] = data_mesin[i]['qty'];
        isi_map['e'] = data_mesin[i]['harga_so'];
        isi_map['f'] = data_mesin[i]['sub_total'];
        isi_detail_excel.add(isi_map);
      }
      footer_excel.add("");
      footer_excel.add("");
      footer_excel.add("Jumlah");
      footer_excel.add(data_pomesin_list[index_terpilih]['total_qty']);
      footer_excel.add("Total");
      footer_excel.add(data_pomesin_list[index_terpilih]['total_so']);
      String judul =
          "Invoice Order Penjualan (${data_pomesin_list[index_terpilih]['no_bukti']})";
  ...
  Future<void> proses_print() async {
    List data_mesin = await m_pomesin.select_po_mesin_detail(
        data_pomesin_list[index_terpilih]['no_bukti'], "NO_BUKTI", "po");
    InvoiceOrderPenjualan()
        .proses_print(data_pomesin_list[index_terpilih], data_mesin);
  }
  ...
  Future<void> initData_addPomesin() async {
    ...
    await m_pomesin
      ...
      if (value != null) {
        ...
        nobuktiController.text =
            "PO${format_no_bukti.format(DateTime.now())}IM-${no_urut + 1}";
      }
    });
  ...
  ///HEADER///
  Future<void> initData_editPomesin(var data_edit) async {
    ...
    List data_lama = await m_pomesin.select_po_mesin_detail(
  ...
  /// data header
  Future<bool> savePomesin() async {
    hitungSubTotal();
    if (nobuktiController.text.isNotEmpty) {
      if (data_brg_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_pomesin.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_pomesin.insert_po_mesin(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editPomesin() async {
    ...
    if (nobuktiController.text.isNotEmpty) {
      if (data_brg_keranjang.length > 0) {
        ...
        obj['total'] = sumTotal;
        obj['tabeld'] = await baca_tabeld();
        await m_pomesin.update_po_mesin(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deletePomesin(String no_bukti) async {
    try {
      var delete = await m_pomesin.delete_po_mesin(no_bukti);
      await select_data();
      return true;
    } catch (e) {
      return false;
