...
import 'package:akunt/model/model_poimport.dart';
import 'package:akunt/view/base_widget/toast.dart';
import 'package:syncfusion_flutter_datepicker/datepicker.dart';

class PoimportController with ChangeNotifier {
  model_poimport m_poimport = model_poimport();
  TextEditingController searchController = TextEditingController();
  DateRangePickerController filter_tanggalController =
      new DateRangePickerController();
  List data_poimport_list = [];
  ...
  void modalData(String cari) async {
    data_poimport_list = await model_poimport().data_modal(cari);
    notifyListeners();
  }

  Future<void> select_data() async {
    data_poimport_list = await m_poimport.select_po_import(
        searchController.text, tanggal_awal, tanggal_akhir);
    notifyListeners();
  }

  void selectData(String cari) async {
    data_poimport_list = await model_poimport().cari_po_import(cari);
    notifyListeners();
  }
  ...
  void proses_export() {
    if (data_poimport_list.length > 0) {
      ...
      for (int i = 0; i < data_poimport_list.length; i++) {
        Map<String, dynamic> isi_map = new Map<String, dynamic>();
        isi_map['a'] = data_poimport_list[i]['tanggal'];
        isi_map['b'] = data_poimport_list[i]['no_bukti'];
        isi_map['c'] = data_poimport_list[i]['sales'];
        isi_map['d'] = data_poimport_list[i]['customer'];
        isi_map['e'] = data_poimport_list[i]['keterangan'];
        isi_map['f'] = data_poimport_list[i]['total_qty'];
        isi_map['g'] = data_poimport_list[i]['total_so'];
        if (data_poimport_list[i]['status'] == 1) {
          isi_map['h'] = "Diterima";
        } else {
  ...
  Future<void> proses_export_detail() async {
    if (index_terpilih != null) {
      ...
      map_transaksi['a'] = data_poimport_list[index_terpilih]['tanggal'];
      map_transaksi['b'] = data_poimport_list[index_terpilih]['no_bukti'];
      map_transaksi['c'] = data_poimport_list[index_terpilih]['sales'];
      map_transaksi['d'] = data_poimport_list[index_terpilih]['customer'];
      map_transaksi['e'] = data_poimport_list[index_terpilih]['keterangan'];
      if (data_poimport_list[index_terpilih]['status'] == 1) {
      ...
      List data_account = await m_poimport.select_po_import_detail(
          data_poimport_list[index_terpilih]['no_bukti'], "NO_BUKTI", "kas");
      for (int i = 0; i < data_account.length; i++) {
        Map<String, dynamic> isi_map = new Map<String, dynamic>();
        isi_map['a'] = data_account[i]['kd'];
      ...
      footer_excel.add(data_poimport_list[index_terpilih]['total_qty']);
      footer_excel.add("Total");
      footer_excel.add(data_poimport_list[index_terpilih]['total_so']);
      String judul =
          "Invoice Order Penjualan (${data_poimport_list[index_terpilih]['no_bukti']})";
  ...
  Future<void> proses_print() async {
    List data_account = await m_poimport.select_po_import_detail(
        data_poimport_list[index_terpilih]['no_bukti'], "NO_BUKTI", "po");
    InvoiceOrderPenjualan()
        .proses_print(data_poimport_list[index_terpilih], data_account);
  }
  ...
  Future<void> initData_addPoimport() async {
    ...
    await m_poimport
      ...
      if (value != null) {
        ...
        nobuktiController.text =
            "PO${format_no_bukti.format(DateTime.now())}IMP-${no_urut + 1}";
      }
    });
  ...
  ///HEADER///
  Future<void> initData_editPoimport(var data_edit) async {
    ...
    List data_lama = await m_poimport.select_po_import_detail(
  ...
  /// data header
  Future<bool> savePoimport() async {
    hitungSubTotal();
    if (nobuktiController.text.isNotEmpty) {
      if (data_brg_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_poimport.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_poimport.insert_po_import(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editPoimport() async {
    hitungSubTotal();
    if (nobuktiController.text.isNotEmpty) {
      if (data_brg_keranjang.length > 0) {
        BotToast.showLoading();
        ...
        obj['total'] = sumTotal;
        obj['tabeld'] = await baca_tabeld();
        await m_poimport.update_po_import(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deletePoimport(String no_bukti) async {
    try {
      var delete = await m_poimport.delete_po_import(no_bukti);
      await select_data();
      return true;
    } catch (e) {
      return false;
  ...
  Future<List> baca_tabeld() async {
    ...
    for (int i = 0; i < data_brg_keranjang.length; i++) {
      ...
      Map obj = new Map();
      obj['kd'] = data_brg_keranjang[i].kd_brg;
