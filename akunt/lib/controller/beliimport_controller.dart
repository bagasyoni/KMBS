...
import 'package:akunt/model/model_beliimport.dart';
import 'package:akunt/view/base_widget/toast.dart';
import 'package:syncfusion_flutter_datepicker/datepicker.dart';

class BeliimportController with ChangeNotifier {
  model_beliimport m_beliimport = model_beliimport();
  TextEditingController searchController = TextEditingController();
  DateRangePickerController filter_tanggalController =
      new DateRangePickerController();
  List data_beliimport_list = [];
  ...
  void selectData(String cari) async {
    data_beliimport_list = await model_beliimport().cari_beliimport(cari);
    notifyListeners();
  }

  Future<void> select_data() async {
    data_beliimport_list = await m_beliimport.select_beli_import(
  ...
  void proses_export() {
    if (data_beliimport_list.length > 0) {
      ...
      for (int i = 0; i < data_beliimport_list.length; i++) {
        Map<String, dynamic> isi_map = new Map<String, dynamic>();
        isi_map['a'] = data_beliimport_list[i]['tanggal'];
        isi_map['b'] = data_beliimport_list[i]['no_bukti'];
        isi_map['c'] = data_beliimport_list[i]['sales'];
        isi_map['d'] = data_beliimport_list[i]['customer'];
        isi_map['e'] = data_beliimport_list[i]['keterangan'];
        isi_map['f'] = data_beliimport_list[i]['total_qty'];
        isi_map['g'] = data_beliimport_list[i]['total_so'];
        if (data_beliimport_list[i]['status'] == 1) {
          isi_map['h'] = "Diterima";
        } else {
  ...
  Future<void> proses_export_detail() async {
    if (index_terpilih != null) {
      ...
      map_transaksi['a'] = data_beliimport_list[index_terpilih]['tanggal'];
      map_transaksi['b'] = data_beliimport_list[index_terpilih]['no_bukti'];
      map_transaksi['c'] = data_beliimport_list[index_terpilih]['sales'];
      map_transaksi['d'] = data_beliimport_list[index_terpilih]['customer'];
      map_transaksi['e'] = data_beliimport_list[index_terpilih]['keterangan'];
      if (data_beliimport_list[index_terpilih]['status'] == 1) {
      ...
      List data_bahan = await m_beliimport.select_beli_import_detail(
          data_beliimport_list[index_terpilih]['no_bukti'], "NO_BUKTI", "beli");
      ...
      footer_excel.add(data_beliimport_list[index_terpilih]['total_qty']);
      footer_excel.add("Total");
      footer_excel.add(data_beliimport_list[index_terpilih]['total_so']);
      String judul =
          "Invoice Order Penjualan (${data_beliimport_list[index_terpilih]['no_bukti']})";
  ...
  Future<void> proses_print() async {
    ...
    InvoiceOrderPenjualan()
        .proses_print(data_beliimport_list[index_terpilih], data_bahan);
  }
  ...
  Future<void> initData_addBeliimport() async {
    ...
    await m_beliimport
      ...
      if (value != null) {
        ...
        nobuktiController.text =
            "BL${format_no_bukti.format(DateTime.now())}IMP-${no_urut + 1}";
      }
    });
    await model_pod().cari_pod_import("").then((value) {
      if (value != null) {
        podList.clear();
  ...
  ///HEADER///
  Future<void> initData_editBeliimport(var data_edit) async {
    ...
    List data_lama = await m_beliimport.select_beli_import_detail(
  ...
  /// data header
  Future<bool> saveBeliimport() async {
    hitungSubTotal();
    if (nobuktiController.text.isNotEmpty) {
      if (data_pod_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_beliimport.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_beliimport.insert_beli_import(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editBeliimport() async {
    hitungSubTotal();
    if (nobuktiController.text.isNotEmpty) {
      if (data_pod_keranjang.length > 0) {
        BotToast.showLoading();
        ...
        obj['nett'] = sumTotal;
        obj['tabeld'] = await baca_tabeld();
        await m_beliimport.update_beli_import(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteBeliimport(String no_bukti) async {
    try {
      var delete = await m_beliimport.delete_beli_import(no_bukti);
      await select_data();
      return true;
    } catch (e) {
      return false;
