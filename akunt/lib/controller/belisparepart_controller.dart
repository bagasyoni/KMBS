...
import 'package:akunt/model/model_belisparepart.dart';
import 'package:akunt/view/base_widget/toast.dart';
import 'package:syncfusion_flutter_datepicker/datepicker.dart';

class BelisparepartController with ChangeNotifier {
  model_belisparepart m_belisparepart = model_belisparepart();
  TextEditingController searchController = TextEditingController();
  DateRangePickerController filter_tanggalController =
      new DateRangePickerController();
  List data_belisparepart_list = [];
  ...
  void selectData(String cari) async {
    data_belisparepart_list =
        await model_belisparepart().cari_belisparepart(cari);
    notifyListeners();
  }

  Future<void> select_data() async {
    data_belisparepart_list = await m_belisparepart.select_beli_sparepart(
  ...
  void proses_export() {
    if (data_belisparepart_list.length > 0) {
      ...
      for (int i = 0; i < data_belisparepart_list.length; i++) {
        Map<String, dynamic> isi_map = new Map<String, dynamic>();
        isi_map['a'] = data_belisparepart_list[i]['tanggal'];
        isi_map['b'] = data_belisparepart_list[i]['no_bukti'];
        isi_map['c'] = data_belisparepart_list[i]['sales'];
        isi_map['d'] = data_belisparepart_list[i]['customer'];
        isi_map['e'] = data_belisparepart_list[i]['keterangan'];
        isi_map['f'] = data_belisparepart_list[i]['total_qty'];
        isi_map['g'] = data_belisparepart_list[i]['total_so'];
        if (data_belisparepart_list[i]['status'] == 1) {
          isi_map['h'] = "Diterima";
        } else {
  ...
  Future<void> proses_export_detail() async {
    if (index_terpilih != null) {
      ...
      map_transaksi['a'] = data_belisparepart_list[index_terpilih]['tanggal'];
      map_transaksi['b'] = data_belisparepart_list[index_terpilih]['no_bukti'];
      map_transaksi['c'] = data_belisparepart_list[index_terpilih]['sales'];
      map_transaksi['d'] = data_belisparepart_list[index_terpilih]['customer'];
      map_transaksi['e'] =
          data_belisparepart_list[index_terpilih]['keterangan'];
      if (data_belisparepart_list[index_terpilih]['status'] == 1) {
      ...
      List data_sparepart = await m_belisparepart.select_beli_sparepart_detail(
          data_belisparepart_list[index_terpilih]['no_bukti'],
          "NO_BUKTI",
          "beli");
      ...
      footer_excel.add(data_belisparepart_list[index_terpilih]['total_qty']);
      footer_excel.add("Total");
      footer_excel.add(data_belisparepart_list[index_terpilih]['total_so']);
      String judul =
          "Invoice Order Penjualan (${data_belisparepart_list[index_terpilih]['no_bukti']})";
  ...
  Future<void> proses_print() async {
    ...
    InvoiceOrderPenjualan()
        .proses_print(data_belisparepart_list[index_terpilih], data_sparepart);
  }
  ...
  Future<void> initData_addBelisparepart() async {
    ...
    await m_belisparepart
      ...
      if (value != null) {
        ...
        nobuktiController.text =
            "BL${format_no_bukti.format(DateTime.now())}SP-${no_urut + 1}";
      }
    });
    await model_pod().cari_pod_sparepart("").then((value) {
      if (value != null) {
        podList.clear();
  ...
  ///HEADER///
  Future<void> initData_editBelisparepart(var data_edit) async {
    ...
    List data_lama = await m_belisparepart.select_beli_sparepart_detail(
  ...
  /// data header
  Future<bool> saveBelisparepart() async {
    hitungSubTotal();
    if (nobuktiController.text.isNotEmpty) {
      if (data_pod_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_belisparepart.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_belisparepart.insert_beli_sparepart(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editBelisparepart() async {
    hitungSubTotal();
    if (nobuktiController.text.isNotEmpty) {
      if (data_pod_keranjang.length > 0) {
        BotToast.showLoading();
        ...
        obj['nett'] = sumTotal;
        obj['tabeld'] = await baca_tabeld();
        await m_belisparepart.update_beli_sparepart(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteBelisparepart(String no_bukti) async {
    try {
      var delete = await m_belisparepart.delete_beli_sparepart(no_bukti);
      await select_data();
      return true;
    } catch (e) {
      return false;
