...
import 'package:akunt/model/model_belimesin.dart';
import 'package:akunt/view/base_widget/toast.dart';
import 'package:syncfusion_flutter_datepicker/datepicker.dart';

class BelimesinController with ChangeNotifier {
  model_belimesin m_belimesin = model_belimesin();
  TextEditingController searchController = TextEditingController();
  DateRangePickerController filter_tanggalController =
      new DateRangePickerController();
  List data_belimesin_list = [];
  ...
  void selectData(String cari) async {
    data_belimesin_list = await model_belimesin().cari_belimesin(cari);
    notifyListeners();
  }

  Future<void> select_data() async {
    data_belimesin_list = await m_belimesin.select_beli_mesin(
  ...
  void proses_export() {
    if (data_belimesin_list.length > 0) {
      ...
      for (int i = 0; i < data_belimesin_list.length; i++) {
        Map<String, dynamic> isi_map = new Map<String, dynamic>();
        isi_map['a'] = data_belimesin_list[i]['tanggal'];
        isi_map['b'] = data_belimesin_list[i]['no_bukti'];
        isi_map['c'] = data_belimesin_list[i]['sales'];
        isi_map['d'] = data_belimesin_list[i]['customer'];
        isi_map['e'] = data_belimesin_list[i]['keterangan'];
        isi_map['f'] = data_belimesin_list[i]['total_qty'];
        isi_map['g'] = data_belimesin_list[i]['total_so'];
        if (data_belimesin_list[i]['status'] == 1) {
          isi_map['h'] = "Diterima";
        } else {
  ...
  Future<void> proses_export_detail() async {
    if (index_terpilih != null) {
      ...
      map_transaksi['a'] = data_belimesin_list[index_terpilih]['tanggal'];
      map_transaksi['b'] = data_belimesin_list[index_terpilih]['no_bukti'];
      map_transaksi['c'] = data_belimesin_list[index_terpilih]['sales'];
      map_transaksi['d'] = data_belimesin_list[index_terpilih]['customer'];
      map_transaksi['e'] = data_belimesin_list[index_terpilih]['keterangan'];
      if (data_belimesin_list[index_terpilih]['status'] == 1) {
      ...
      List data_mesin = await m_belimesin.select_beli_mesin_detail(
          data_belimesin_list[index_terpilih]['no_bukti'], "NO_BUKTI", "beli");
      ...
      footer_excel.add(data_belimesin_list[index_terpilih]['total_qty']);
      footer_excel.add("Total");
      footer_excel.add(data_belimesin_list[index_terpilih]['total_so']);
      String judul =
          "Invoice Order Penjualan (${data_belimesin_list[index_terpilih]['no_bukti']})";
  ...
  Future<void> proses_print() async {
    ...
    InvoiceOrderPenjualan()
        .proses_print(data_belimesin_list[index_terpilih], data_mesin);
  }
  ...
  Future<void> initData_addBelimesin() async {
    ...
    await m_belimesin
      ...
      if (value != null) {
        ...
        nobuktiController.text =
            "BL${format_no_bukti.format(DateTime.now())}IM-${no_urut + 1}";
      }
    });
    await model_pod().cari_pod_mesin("").then((value) {
      if (value != null) {
        podList.clear();
  ...
  ///HEADER///
  Future<void> initData_editBelimesin(var data_edit) async {
    ...
    List data_lama = await m_belimesin.select_beli_mesin_detail(
  ...
  /// data header
  Future<bool> saveBelimesin() async {
    hitungSubTotal();
    if (nobuktiController.text.isNotEmpty) {
      if (data_pod_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_belimesin.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_belimesin.insert_beli_mesin(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editBelimesin() async {
    hitungSubTotal();
    if (nobuktiController.text.isNotEmpty) {
      if (data_pod_keranjang.length > 0) {
        BotToast.showLoading();
        ...
        obj['nett'] = sumTotal;
        obj['tabeld'] = await baca_tabeld();
        await m_belimesin.update_beli_mesin(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteBelimesin(String no_bukti) async {
    try {
      var delete = await m_belimesin.delete_beli_mesin(no_bukti);
      await select_data();
      return true;
    } catch (e) {
      return false;
