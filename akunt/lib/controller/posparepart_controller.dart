...
import 'package:akunt/model/model_posparepart.dart';
import 'package:akunt/view/base_widget/toast.dart';
import 'package:syncfusion_flutter_datepicker/datepicker.dart';

class PosparepartController with ChangeNotifier {
  model_posparepart m_posparepart = model_posparepart();
  TextEditingController searchController = TextEditingController();
  DateRangePickerController filter_tanggalController =
      new DateRangePickerController();
  List data_posparepart_list = [];
  ...
  void modalData(String cari) async {
    data_posparepart_list = await model_posparepart().data_modal(cari);
    notifyListeners();
  }

  Future<void> select_data() async {
    data_posparepart_list = await m_posparepart.select_po_sparepart(
        searchController.text, tanggal_awal, tanggal_akhir);
    notifyListeners();
  }

  void selectData(String cari) async {
    data_posparepart_list = await model_posparepart().cari_po_sparepart(cari);
    notifyListeners();
  }
  ...
  void proses_export() {
    if (data_posparepart_list.length > 0) {
      ...
      for (int i = 0; i < data_posparepart_list.length; i++) {
        Map<String, dynamic> isi_map = new Map<String, dynamic>();
        isi_map['a'] = data_posparepart_list[i]['tanggal'];
        isi_map['b'] = data_posparepart_list[i]['no_bukti'];
        isi_map['c'] = data_posparepart_list[i]['sales'];
        isi_map['d'] = data_posparepart_list[i]['customer'];
        isi_map['e'] = data_posparepart_list[i]['keterangan'];
        isi_map['f'] = data_posparepart_list[i]['total_qty'];
        isi_map['g'] = data_posparepart_list[i]['total_so'];
        if (data_posparepart_list[i]['status'] == 1) {
          isi_map['h'] = "Diterima";
        } else {
  ...
  Future<void> proses_export_detail() async {
    if (index_terpilih != null) {
      ...
      map_transaksi['a'] = data_posparepart_list[index_terpilih]['tanggal'];
      map_transaksi['b'] = data_posparepart_list[index_terpilih]['no_bukti'];
      map_transaksi['c'] = data_posparepart_list[index_terpilih]['sales'];
      map_transaksi['d'] = data_posparepart_list[index_terpilih]['customer'];
      map_transaksi['e'] = data_posparepart_list[index_terpilih]['keterangan'];
      if (data_posparepart_list[index_terpilih]['status'] == 1) {
      ...
      List data_sparepart = await m_posparepart.select_po_sparepart_detail(
          data_posparepart_list[index_terpilih]['no_bukti'], "NO_BUKTI", "kas");
      for (int i = 0; i < data_sparepart.length; i++) {
        Map<String, dynamic> isi_map = new Map<String, dynamic>();
        isi_map['a'] = data_sparepart[i]['kd_brg'];
        isi_map['b'] = data_sparepart[i]['na_brg'];
        isi_map['c'] = data_sparepart[i]['satuan'];
        isi_map['d'] = data_sparepart[i]['qty'];
        isi_map['e'] = data_sparepart[i]['harga_so'];
        isi_map['f'] = data_sparepart[i]['sub_total'];
        isi_detail_excel.add(isi_map);
      }
      footer_excel.add("");
      footer_excel.add("");
      footer_excel.add("Jumlah");
      footer_excel.add(data_posparepart_list[index_terpilih]['total_qty']);
      footer_excel.add("Total");
      footer_excel.add(data_posparepart_list[index_terpilih]['total_so']);
      String judul =
          "Invoice Order Penjualan (${data_posparepart_list[index_terpilih]['no_bukti']})";
  ...
  Future<void> proses_print() async {
    List data_sparepart = await m_posparepart.select_po_sparepart_detail(
        data_posparepart_list[index_terpilih]['no_bukti'], "NO_BUKTI", "po");
    InvoiceOrderPenjualan()
        .proses_print(data_posparepart_list[index_terpilih], data_sparepart);
  }
  ...
  Future<void> initData_addPosparepart() async {
    ...
    await m_posparepart
      ...
      if (value != null) {
        ...
        nobuktiController.text =
            "PO${format_no_bukti.format(DateTime.now())}SP-${no_urut + 1}";
      }
    });
  ...
  ///HEADER///
  Future<void> initData_editPosparepart(var data_edit) async {
    ...
    List data_lama = await m_posparepart.select_po_sparepart_detail(
    ...
    for (int i = 0; i < data_lama.length; i++) {
      DataBrg mAccount = DataBrg(
        ...
        kd_brg: data_lama[i]['KD_BHN'],
        na_brg: data_lama[i]['NA_BHN'],
  ...
  /// data header
  Future<bool> savePosparepart() async {
    hitungSubTotal();
    if (nobuktiController.text.isNotEmpty) {
      if (data_brg_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_posparepart.get_no_bukti(
        ...
        if (data_ready.length > 0) {
          ...
          await m_posparepart.insert_po_sparepart(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editPosparepart() async {
    hitungSubTotal();
    if (nobuktiController.text.isNotEmpty) {
      if (data_brg_keranjang.length > 0) {
        BotToast.showLoading();
        ...
        obj['total'] = sumTotal;
        obj['tabeld'] = await baca_tabeld();
        await m_posparepart.update_po_sparepart(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deletePosparepart(String no_bukti) async {
    try {
      var delete = await m_posparepart.delete_po_sparepart(no_bukti);
      await select_data();
      return true;
    } catch (e) {
      return false;
