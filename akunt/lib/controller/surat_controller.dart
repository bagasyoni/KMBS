...
class SuratController with ChangeNotifier {
  ...
  void selectData(String cari) async {
    data_surat_list = await model_surat().cari_surat(cari);
    notifyListeners();
  }

  Future<void> select_data() async {
    data_surat_list = await m_surat.select_surat(
  ...
  void proses_export() {
    if (data_surat_list.length > 0) {
      ...
      for (int i = 0; i < data_surat_list.length; i++) {
        Map<String, dynamic> isi_map = new Map<String, dynamic>();
        isi_map['a'] = data_surat_list[i]['tanggal'];
        isi_map['b'] = data_surat_list[i]['no_bukti'];
        isi_map['c'] = data_surat_list[i]['sales'];
        isi_map['d'] = data_surat_list[i]['customer'];
        isi_map['e'] = data_surat_list[i]['keterangan'];
        isi_map['f'] = data_surat_list[i]['total_qty'];
        isi_map['g'] = data_surat_list[i]['total_so'];
        if (data_surat_list[i]['status'] == 1) {
          isi_map['h'] = "Diterima";
        } else {
  ...
  Future<void> proses_export_detail() async {
    if (index_terpilih != null) {
      ...
      map_transaksi['a'] = data_surat_list[index_terpilih]['tanggal'];
      map_transaksi['b'] = data_surat_list[index_terpilih]['no_bukti'];
      map_transaksi['c'] = data_surat_list[index_terpilih]['sales'];
      map_transaksi['d'] = data_surat_list[index_terpilih]['customer'];
      map_transaksi['e'] = data_surat_list[index_terpilih]['keterangan'];
      if (data_surat_list[index_terpilih]['status'] == 1) {
      ...
      List data_barang = await m_surat.select_surat_detail(
          data_surat_list[index_terpilih]['no_bukti'], "NO_BUKTI", "surat");
      ...
      footer_excel.add(data_surat_list[index_terpilih]['total_qty']);
      footer_excel.add("Total");
      footer_excel.add(data_surat_list[index_terpilih]['total_so']);
      String judul =
          "Invoice Order Penjualan (${data_surat_list[index_terpilih]['no_bukti']})";
  ...
  Future<void> proses_print() async {
    ...
    InvoiceOrderPenjualan()
        .proses_print(data_surat_list[index_terpilih], data_barang);
  }
  ...
  TextEditingController nosoController = TextEditingController();
  ...
  Future<void> initData_addSurat() async {
    ...
    await m_surat
      ...
      if (value != null) {
        ...
        no_buktiController.text =
            "SJ${format_no_bukti.format(DateTime.now())}BR-${no_urut + 1}";
      }
    });
  }

  ///HEADER///
  Future<void> initData_editSurat(var data_edit) async {
    no_buktiController.text = data_edit['NO_BUKTI'];
    tglController.text = data_edit['TGL'];
    jtempoController.text = data_edit['JTEMPO'];
    nosoController.text = data_edit['NO_SO'];
    ...
    List data_lama = await m_surat.select_surat_detail(
        data_edit['NO_BUKTI'], "NO_BUKTI", "suratd");
    data_brg_keranjang = new List<DataBrg>();
  ...
  /// data header
  Future<bool> saveSurat() async {
    hitungSubTotal();
    if (no_buktiController.text.isNotEmpty) {
      if (data_brg_keranjang.length > 0) {
        BotToast.showLoading();
        var data_ready = await m_surat.get_no_bukti(
            no_buktiController.text, "NO_BUKTI", "surat");
        if (data_ready.length > 0) {
          ...
          await m_surat.insert_surat(obj);
          BotToast.closeAllLoading();
          return true;
        }
      } else {
  ...
  Future<bool> editSurat() async {
    ...
    if (no_buktiController.text.isNotEmpty) {
      if (data_brg_keranjang.length > 0) {
        ...
        obj['TOTAL'] = sumTotal;
        obj['tabeld'] = await baca_tabeld();
        await m_surat.update_surat(obj);
        BotToast.closeAllLoading();
  ...
  Future<bool> deleteSurat(String no_bukti) async {
    try {
      var delete = await m_surat.delete_surat(no_bukti);
      await select_data();
      return true;
    } catch (e) {
      return false;
  ...
  Future<void> importDataFromSO(var data_po) async {
    nosoController.text = data_po['NO_BUKTI'].toString();
    kodecController.text = data_po['KODEC'].toString();
    namacController.text = data_po['NAMAC'].toString();
    tglController.text = data_po['TGL'].toString();
    List data_detail_so = await model_so()
        .select_so_detail(data_po['NO_BUKTI'], "NO_BUKTI", "sod");
    data_brg_keranjang = [];
    for (int i = 0; i < data_detail_so.length; i++) {
      DataBrg m_bhn = DataBrg(
        noid: int.parse(data_detail_so[i]['NO_ID'].toString()),
        kd_brg: data_detail_so[i]['KD_BRG'],
        na_brg: data_detail_so[i]['NA_BRG'],
        satuan: data_detail_so[i]['SATUAN'],
        ket: data_detail_so[i]['KET'],
        harga: double.parse(data_detail_so[i]['HARGA'].toString()),
        qty: double.parse(data_detail_so[i]['QTY'].toString()),
      );
