...
import '../model/model_lapthutbahan.dart';

class LapThutbahanController with ChangeNotifier {
  final Future<SharedPreferences> _prefs = SharedPreferences.getInstance();
  SharedPreferences prefs;
  model_lapthutbahan m_lapthutbahan = model_lapthutbahan();
  ...
  Future<void> select_data() async {
    data_list = await m_lapthutbahan.data_lap(
  ...
  Future<void> proses_export_lapthutbahan(int mode) async {
    if (data_list.length > 0) {
      ...
      for (int i = 0; i < data_list.length; i++) {
        ...
        isi_map['c'] = data_list[i]['JTEMPO'];
        isi_map['d'] = data_list[i]['KODES'];
        isi_map['e'] = data_list[i]['NAMAS'];
        isi_map['f'] = data_list[i]['KOTA'];
        isi_map['g'] = data_list[i]['TOTAL'];
        isi_excel.add(isi_map);
      }
      String judul = "Laporan Transaksi Hutang Bahan";
      String header_title = "";
      if (mode == 0) {
