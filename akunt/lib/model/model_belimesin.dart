...
class model_belimesin {
  ...
  Future<List> insert_beli_mesin(Map data_insert) async {
    try {
      ///DATA HEADER
      ///AMBIL DARI CONTROLLER
      final response = await http.post(
        Uri.parse("${baseUrl}:3000/tambah_header_beli_mesin"),
        body: {
      ...
      for (int i = 0; i < data_detail.length; i++) {
        await http.post(
          Uri.parse("${baseUrl}:3000/tambah_detail_beli_nonmesin"),
          body: {
            "rec": (i + 1).toString(),
            "nobukti": data_insert['nobukti'].toString(),
  ...
  Future<List> update_beli_mesin(Map data_insert) async {
    try {
      await http.post(
      ...
      ///AMBIL DARI CONTROLLER
      final response = await http.post(
        Uri.parse("${baseUrl}:3000/edit_header_beli_mesin"),
        body: {
          "nobukti": data_insert['nobukti'].toString(),
          "dr": data_insert['dr'].toString(),
          "nopo": data_insert['nopo'].toString(),
      ...
      for (int i = 0; i < data_detail.length; i++) {
        await http.post(
          Uri.parse("${baseUrl}:3000/tambah_detail_beli_mesin"),
          body: {
            "rec": (i + 1).toString(),
            "nobukti": data_insert['nobukti'].toString(),
            "kdbhn": data_detail[i]['kdbhn'].toString(),
  ...
  Future<List> cari_belimesin(String key_cari) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/cari_beli_mesin"),
      body: {"cari": key_cari},
    );
    var results2 = json.decode(response.body);
    return results2['data'].toList();
  ...
  ///SELECT HEADER
  Future<List> select_beli_mesin(
      String cari, String start_date, String end_date) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/tampil_beli_mesin"),
  ...
  Future<List> select_beli_mesin_aktif(String cari, String sales,
  ...
  ///SELECT DETAIL
  Future<List> select_beli_mesin_detail(
  ...
  Future<List> delete_beli_mesin(String no_bukti) async {
    var konek = await m_koneksi.koneksi();
    var results1 =
