...
class model_dragonpo {
  ...
  Future<List> insert_po(Map data_insert) async {
    try {
      ///DATA HEADER
      ///AMBIL DARI CONTROLLER
      final response = await http.post(
        Uri.parse("${baseUrl}:3000/tambah_header_po"),
        body: {
      ...
      for (int i = 0; i < data_detail.length; i++) {
        await http.post(
          Uri.parse("${baseUrl}:3000/tambah_detail_po"),
          body: {
            "rec": (i + 1).toString(),
            "nobukti": data_insert['no_bukti'].toString(),
            "per": "01/2022",
  ...
  Future<List> update_po(Map data_insert) async {
    try {
      await http.post(
      ...
      ///AMBIL DARI CONTROLLER
      final response = await http.post(
        Uri.parse("${baseUrl}:3000/edit_header_po"),
        body: {
          "nobukti": data_insert['no_bukti'].toString(),
          "dragon": data_insert['dragon'].toString(),
          "kodes": data_insert['kodes'].toString(),
  ...
  ///SELECT HEADER
  Future<List> select_dragonpo(
      String cari, String start_date, String end_date) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/tampil_pod"),
  ...
  Future<List> select_po_aktif(String cari, String sales, String customer,
  ...
  Future<List> delete_po(String no_bukti) async {
    var konek = await m_koneksi.koneksi();
    var results1 =
