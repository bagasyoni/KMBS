...
class model_beliimport {
  ...
  Future<List> insert_beli_import(Map data_insert) async {
    try {
      ///DATA HEADER
      ///AMBIL DARI CONTROLLER
      final response = await http.post(
        Uri.parse("${baseUrl}:3000/tambah_header_beli_import"),
        body: {
      ...
      for (int i = 0; i < data_detail.length; i++) {
        await http.post(
          Uri.parse("${baseUrl}:3000/tambah_detail_beli_import"),
          body: {
            "rec": (i + 1).toString(),
            "nobukti": data_insert['nobukti'].toString(),
  ...
  Future<List> update_beli_import(Map data_insert) async {
    try {
      await http.post(
      ...
      ///AMBIL DARI CONTROLLER
      final response = await http.post(
        Uri.parse("${baseUrl}:3000/edit_header_beli_import"),
        body: {
          "nobukti": data_insert['nobukti'].toString(),
          "dr": data_insert['dr'].toString(),
          "nopo": data_insert['nopo'].toString(),
  ...
  Future<List> cari_beliimport(String key_cari) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/cari_beli_import"),
      body: {"cari": key_cari},
    );
    var results2 = json.decode(response.body);
    return results2['data'].toList();
  ...
  Future<List> select_beli_import(
      String cari, String start_date, String end_date) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/tampil_beli_import"),
  ...
  Future<List> select_beli_import_aktif(String cari, String sales,
  ...
  ///SELECT DETAIL
  Future<List> select_beli_import_detail(
  ...
  Future<List> delete_beli_import(String no_bukti) async {
    var konek = await m_koneksi.koneksi();
    var results1 =
