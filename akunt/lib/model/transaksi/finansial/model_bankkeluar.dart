...
class model_bankk {
  ...
  /// paginate
  Future<List> data_bankkpaginate(
      String cari, int paramoffset, int paramlimit) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/bankkpaginate"),
      body: {
        "cari": cari,
  ...
  ///paginate
  Future countBankkPaginate(String key_cari) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/countbankkpaginate"),
      body: {"cari": key_cari},
    );
    var results2 = json.decode(response.body);
    return results2['data'].toList();
  ...
  Future<List> data_modal(String cari) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/modal_bankk"),
      body: {"cari": cari},
    );
    var results2 = json.decode(response.body);
    return results2['data'].toList();
  ...
  Future<List> insert_bankk(Map data_insert) async {
    try {
      final response = await http.post(
        Uri.parse("${baseUrl}:3000/tambah_header_bankk"),
        body: {
      ...
      for (int i = 0; i < data_detail.length; i++) {
        await http.post(
          Uri.parse("${baseUrl}:3000/tambah_detail_bankk"),
          body: {
            "REC": (i + 1).toString(),
            "NO_BUKTI": data_insert['NO_BUKTI'].toString(),
            "PER": data_insert['PER'].toString(),
  ...
  Future<List> update_bankk(Map data_insert) async {
    try {
      await http.post(
      ...
      final response = await http.post(
        Uri.parse("${baseUrl}:3000/edit_header_bankk"),
        body: {
          "NO_BUKTI": data_insert['NO_BUKTI'].toString(),
          "TGL": data_insert['TGL'].toString(),
          "TYPE": data_insert['TYPE'].toString(),
  ...
  Future<List> cari_bankk(String cari) async {
    final response = await http.post(
  ...
  ///SELECT DETAIL
  Future<List> select_bankk_detail(
  ...
  Future<List> delete_bankk(String no_bukti) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/hapus_bankk"),
