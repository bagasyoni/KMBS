...
class model_btbbahanlokal {
  String baseUrl = base_url;

  /// paginate
  Future<List> data_btbbahanlokalpaginate(
      String cari, int paramoffset, int paramlimit) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/btbbahanlokalpaginate"),
      body: {
        "cari": cari,
  ...
  ///paginate
  Future countBtbbahanlokalPaginate(String key_cari) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/countbtbbahanlokalpaginate"),
      body: {"cari": key_cari},
    );
    var results2 = json.decode(response.body);
    return results2['data'].toList();
  ...
  Future<List> data_modal(String cari) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/modal_btb_bahan_lokal"),
      body: {"cari": cari},
    );
    var results2 = json.decode(response.body);
    return results2['data'].toList();
  ...
  Future<List> insert_btb_bahan_lokal(Map data_insert) async {
    try {
      final response = await http.post(
        Uri.parse("${baseUrl}:3000/tambah_header_btb_bahan_lokal"),
        body: {
      ...
      for (int i = 0; i < data_detail.length; i++) {
        await http.post(
          Uri.parse("${baseUrl}:3000/tambah_detail_btb_bahan_lokal"),
          body: {
            "REC": (i + 1).toString(),
            "NO_BUKTI": data_insert['NO_BUKTI'].toString(),
            "PER": data_insert['PER'].toString(),
  ...
  Future<List> update_btb_bahan_lokal(Map data_insert) async {
    try {
      await http.post(
      ...
      ///DATA HEADER
      final response = await http.post(
        Uri.parse("${baseUrl}:3000/edit_header_btb_bahan_lokal"),
        body: {
          "NO_BUKTI": data_insert['NO_BUKTI'].toString(),
          "TGL": data_insert['TGL'].toString(),
          "JTEMPO": data_insert['JTEMPO'].toString(),
  ...
  Future<List> cari_btb_bahan_lokal(String cari) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/cari_btb_bahan_lokal"),
      body: {"cari": cari},
    );
    var results2 = json.decode(response.body);
    return results2['data'].toList();
  ...
  ///SELECT HEADER
  Future<List> select_btb_bahan_lokal(
      String cari, String start_date, String end_date, String periode) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/tampil_btb_bahan_lokal"),
      body: {
        "cari": cari,
        "tglawal": start_date,
        "tglakhir": end_date,
  ...
  ///SELECT DETAIL
  Future<List> select_btb_bahan_lokal_detail(
  ...
  Future<List> delete_btb_bahan_lokal(String no_bukti) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/hapus_btb_bahanlokal"),
