...
class model_pononbahan {
  ...
  Future<List> data_modal(String cari) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/modal_po_nonbahan"),
      body: {"cari": cari},
    );
  ...
  Future<List> insert_po_nonbahan(Map data_insert) async {
    try {
      ///DATA HEADER
      ///AMBIL DARI CONTROLLER
      final response = await http.post(
        Uri.parse("${baseUrl}:3000/tambah_header_po_nonbahan"),
        body: {
      ...
      for (int i = 0; i < data_detail.length; i++) {
        await http.post(
          Uri.parse("${baseUrl}:3000/tambah_detail_po_nonbahan"),
          body: {
            "rec": (i + 1).toString(),
            "nobukti": data_insert['no_bukti'].toString(),
            "per": "01/2022",
  ...
  Future<List> update_po_nonbahan(Map data_insert) async {
    try {
      await http.post(
      ...
      ///AMBIL DARI CONTROLLER
      final response = await http.post(
        Uri.parse("${baseUrl}:3000/edit_header_po_nonbahan"),
        body: {
          "nobukti": data_insert['no_bukti'].toString(),
          "dragon": data_insert['dragon'].toString(),
          "kodes": data_insert['kodes'].toString(),
  ...
  Future<List> cari_po_nonbahan(String cari) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/cari_po_nonbahan"),
      body: {"cari": cari},
    );
    var results2 = json.decode(response.body);
    return results2['data'].toList();
  ...
  ///SELECT HEADER
  Future<List> select_po_nonbahan(
      String cari, String start_date, String end_date) async {
    final response = await http.post(
      Uri.parse("${baseUrl}:3000/tampil_po_nonbahan"),
  ...
  Future<List> select_po_nonbahan_aktif(String cari, String sales,
  ...
  ///SELECT DETAIL
  Future<List> select_po_nonbahan_detail(
  ...
  Future<List> delete_po_nonbahan(String no_bukti) async {
    var konek = await m_koneksi.koneksi();
    var results1 =
