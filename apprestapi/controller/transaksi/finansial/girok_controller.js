...
exports.count_girokpaginate = function (req, res) {
    var filter_cari = '%' + req.body.cari + '%';
    connection.query("select COUNT(*) from giro where TYPE='BGK' and (NO_BUKTI like ? or BACNO like ? or BNAMA like ? or FLAG='G')", [filter_cari, filter_cari],
        function (error, rows, fields) {
            if (error) {
                console.log(error);
            } else {
...
exports.tampilgirok = function (req, res) {
    ...
    connection.query("select * from giro where if(?<>'',NO_BUKTI like ?,true) AND TGL BETWEEN ? AND ? AND FLAG='G' AND PER=? AND TYPE='BGK'", [nobukti, nobukti, tgl_awal, tgl_akhir, periode], function (error, rows, fields) {
        if (error) {
            console.log(error);
        } else {
