...
///========================/// TRANSAKSI GIRO KELUAR ///========================///
exports.girok_paginate = function (req, res) {
    var filter_cari = '%' + req.body.cari + '%';
    var offset_page = Number(req.body.offset);
    var limit_page = Number(req.body.limit);
    connection.query("select * from giro where NO_BUKTI like ? or BACNO like ? or BNAMA like ? LIMIT ?, ?", [filter_cari, filter_cari, filter_cari, offset_page, limit_page],
...
exports.count_girokpaginate = function (req, res) {
    var filter_cari = '%' + req.body.cari + '%';
    connection.query("select COUNT(*) from giro where TYPE='BGK' and (NO_BUKTI like ? or BACNO like ? or BNAMA like ? or FLAG='G')", [filter_cari, filter_cari, filter_cari],
        function (error, rows, fields) {
            if (error) {
                console.log(error);
            } else {
...
///HEADER
exports.tambahheadergirok = function (req, res) {
...
///DETAIL
exports.tambahdetailgirok = function (req, res) {
    var NO_BUKTI = req.body.NO_BUKTI;
...
exports.tampilgirok = function (req, res) {
    var nobukti = '%' + req.body.cari + '%';
    var tgl_awal = req.body.tglawal;
    var tgl_akhir = req.body.tglakhir;
    var periode = req.body.periode;
    connection.query("select * from giro where if(?<>'',NO_BUKTI like ?,true) AND TGL BETWEEN ? AND ? AND PER=? AND TYPE='BGK'", [nobukti, nobukti, tgl_awal, tgl_akhir, periode], function (error, rows, fields) {
        if (error) {
            console.log(error);
        } else {
...
exports.editheadergirok = function (req, res) {
    var NO_BUKTI = req.body.NO_BUKTI;
    var TGL = req.body.TGL;
    var TYPE = req.body.TYPE;
    var BACNO = req.body.BACNO;
...
exports.modalgirok = function (req, res) {
...
exports.carigirok = function (req, res) {
    var filter_cari = '%' + req.body.cari + '%';
    connection.query("select * from giro where (BACNO like ? or BNAMA like ? or KODE like ? or NAMA like ?) and FLAG='G' and TYPE='BGK'", [filter_cari, filter_cari, filter_cari, filter_cari],
        function (error, rows, fields) {
            if (error) {
                console.log(error);
            } else {
...
exports.hapusgirok = function (req, res) {
    var no_bukti = req.body.no_bukti;
...
exports.ambilgirokdetail = function (req, res) {
    var nobukti = req.body.cari;
