...
///========================/// TRANSAKSI BTB BAHAN LOKAL ///========================///
exports.btbbahanlokal_paginate = function (req, res) {
    var filter_cari = '%' + req.body.cari + '%';
    var offset_page = Number(req.body.offset);
    var limit_page = Number(req.body.limit);
    connection.query("select * from beli where TYP='L' and GOL ='A' and (KODES like ? or NAMAS like ? LIMIT ?, ?)",
    [filter_cari, filter_cari, offset_page, limit_page],
...
exports.count_btbbahanlokalpaginate = function (req, res) {
    var filter_cari = '%' + req.body.cari + '%';
    connection.query("select COUNT(*) from beli where TYP='L' and GOL='A' and (KODES like ? or NAMAS like ?)", [filter_cari, filter_cari],
        function (error, rows, fields) {
            if (error) {
                console.log(error);
            } else {
...
///HEADER
exports.tambahheaderbtbbahanlokal = function (req, res) {
    ...
    connection.query("INSERT INTO beli (NO_BUKTI,TGL,CURR,CURRNM,RATE,KODES,NAMAS,ALAMAT,KOTA,NOTES,TOTAL_QTY,TOTAL1,DISC,PPN,NETT1,SISA,DISC1,PPN1,PPH1,PPH,TOTAL,RPDISC,RPPPN,NETT,RPDISC1,RPPPN1,RPPPH1,RPPPH,USRIN,TG_IN,FLAG,PER,TYP,GOL,NO_SJ,TGL_SJ,INVOICE,NO_FP,TGL_FP,RATEKS,ACNO1,ACNO1_NM,NO_PO,KODE,TOTALBTB) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", [NO_BUKTI, TGL,CURR, CURRNM, RATE, KODES, NAMAS, ALAMAT, KOTA, NOTES, TOTAL_QTY, TOTAL1, DISC, PPN, NETT1, SISA, DISC1, PPN1,PPH1, PPH, TOTAL, RPDISC, RPPPN, NETT, RPDISC1, RPPPN1, RPPPH1, RPPPH, USRIN, TG_IN, FLAG, PER, TYP, GOL, NO_SJ,TGL_SJ, INVOICE, NO_FP, TGL_FP, RATEKS, ACNO1, ACNO1_NM, NO_PO, KODE, TOTALBTB],
        function (error, rows, fields) {
            if (error) {
                ...
                response.ok('Berhasil Tambah Bukti Terima Bahan Lokal Header', res);
...
///DETAIL
exports.tambahdetailbtbbahanlokal = function (req, res) {
    var NO_BUKTI = req.body.NO_BUKTI;
    ...
    connection.query("INSERT INTO belid (NO_BUKTI, REC, PER, FLAG, NO_PO, QTYPO, KD_BHN, NA_BHN, SATUAN, QTY, SATUANBL, QTYBL, HARGA1, TOTAL1, KET, HARGA, TOTAL, BLT, DISC, RPDISC, TYP, GOL, HTG, SIZ, KD, KODECAB, WARNA, PRODUK, GRP, ACNO, ACNO_NM) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?); UPDATE beli, belid SET belid.ID = beli.NO_ID WHERE belid.NO_BUKTI = beli.NO_BUKTI;", [NO_BUKTI, REC, PER, FLAG, NO_PO, QTYPO, KD_BHN, NA_BHN, SATUAN, QTY, SATUANBL, QTYBL, HARGA1, TOTAL1, KET, HARGA, TOTAL, BLT, DISC, RPDISC, TYP, GOL, HTG, SIZ, KD, KODECAB, WARNA, PRODUK, GRP, ACNO, ACNO_NM],
        function (error, rows, fields) {
            if (error) {
                console.log(error);
            } else {

                response.ok('Berhasil Tambah Bukti Terima Bahan Lokal Detail', res);

            }
        });
};

exports.tampilbtbbahanlokal = function (req, res) {
...
exports.editheaderbtbbahanlokal = function (req, res) {
    var NO_BUKTI = req.body.NO_BUKTI;
    var TGL = req.body.TGL;
    var CURR = req.body.CURR;
    var CURRNM = req.body.CURRNM;
    ...
    connection.query("UPDATE beli SET TGL=?, CURR=?, CURRNM=?, RATE=?, KODES=?, NAMAS=?, ALAMAT=?, KOTA=?, NOTES=?, TOTAL_QTY=?, TOTAL1=?, DISC=?, PPN=?, NETT1=?, SISA=?, DISC1=?, PPN1=?, PPH1=?, PPH=?, TOTAL=?, RPDISC=?, RPPPN=?, NETT=?, RPDISC1=?, RPPPN1=?, RPPPH1=?, RPPPH=?, USRIN=?, TG_IN=?, FLAG=?, PER=?, TYP=?, GOL=?, NO_SJ=?,TGL_SJ=?, INVOICE=?, NO_FP=?, TGL_FP=?, RATEKS=?, ACNO1=?, ACNO1_NM=?, NO_PO=?, KODE=?, TOTALBTB=? WHERE NO_BUKTI=?", [TGL,CURR, CURRNM, RATE, KODES, NAMAS, ALAMAT, KOTA, NOTES, TOTAL_QTY, TOTAL1, DISC, PPN, NETT1, SISA, DISC1, PPN1,PPH1, PPH, TOTAL, RPDISC, RPPPN, NETT, RPDISC1, RPPPN1, RPPPH1, RPPPH, USRIN, TG_IN, FLAG, PER, TYP, GOL, NO_SJ,TGL_SJ, INVOICE, NO_FP, TGL_FP, RATEKS, ACNO1, ACNO1_NM, NO_PO, KODE, TOTALBTB, NO_BUKTI],
        function (error, rows, fields) {
            if (error) {
                ...
                response.ok('Berhasil Edit Bukti Terima Bahan Lokal Header', res);

            }
        });
};

exports.modalbtbbahanlokal = function (req, res) {
...
exports.caribtbbahanlokal = function (req, res) {
    var filter_cari = '%' + req.body.cari + '%';
...
exports.hapusbtbbahanlokal = function (req, res) {
    var no_bukti = req.body.no_bukti;
