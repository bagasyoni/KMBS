...
///========================/// TRANSAKSI BTB BAHAN IMPORT ///========================///
exports.btbbahanimport_paginate = function (req, res) {
...
///HEADER
exports.tambahheaderbtbbahanimport = function (req, res) {
    ...
    connection.query("INSERT INTO beli (NO_BUKTI,TGL,CURR,CURRNM,RATE,KODES,NAMAS,ALAMAT,KOTA,NOTES,TOTAL_QTY,TOTAL1,DISC,PPN,NETT1,SISA,DISC1,PPN1,PPH1,PPH,TOTAL,RPDISC,RPPPN,NETT,RPDISC1,RPPPN1,RPPPH1,RPPPH,USRIN,TG_IN,FLAG,PER,TYP,GOL,NO_SJ,TGL_SJ,INVOICE,NO_FP,TGL_FP,RATEKS,ACNO1,ACNO1_NM,NO_PO,KODE,TOTALBTB) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", [NO_BUKTI, TGL, CURR, CURRNM, RATE, KODES, NAMAS, ALAMAT, KOTA, NOTES, TOTAL_QTY, TOTAL1, DISC, PPN, NETT1, SISA, DISC1, PPN1, PPH1, PPH, TOTAL, RPDISC, RPPPN, NETT, RPDISC1, RPPPN1, RPPPH1, RPPPH, USRIN, TG_IN, FLAG, PER, TYP, GOL, NO_SJ, TGL_SJ, INVOICE, NO_FP, TGL_FP, RATEKS, ACNO1, ACNO1_NM, NO_PO, KODE, TOTALBTB],
        function (error, rows, fields) {
            if (error) {
                ...
                response.ok('Berhasil Tambah Bukti Terima Bahan Import Header', res);
...
///DETAIL
exports.tambahdetailbtbbahanimport = function (req, res) {
    var NO_BUKTI = req.body.NO_BUKTI;
    ...
    connection.query("INSERT INTO belid (NO_BUKTI, REC, PER, FLAG, NO_PO, QTYPO, KD_BHN, NA_BHN, SATUAN, QTY, SATUANBL, QTYBL, HARGA1, TOTAL1, KET, HARGA, TOTAL, BLT, DISC, RPDISC, TYP, GOL, HTG, SIZ, KD, KODECAB, WARNA, PRODUK, GRP, ACNO, ACNO_NM) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?); UPDATE beli, belid SET belid.ID = beli.NO_ID WHERE belid.NO_BUKTI = beli.NO_BUKTI;", [NO_BUKTI, REC, PER, FLAG, NO_PO, QTYPO, KD_BHN, NA_BHN, SATUAN, QTY, SATUANBL, QTYBL, HARGA1, TOTAL1, KET, HARGA, TOTAL, BLT, DISC, RPDISC, TYP, GOL, HTG, SIZ, KD, KODECAB, WARNA, PRODUK, GRP, ACNO, ACNO_NM],
        function (error, rows, fields) {
            if (error) {
                ...
                response.ok('Berhasil Tambah Bukti Terima Bahan Import Detail', res);

            }
        });
};

exports.tampilbtbbahanimport = function (req, res) {
    var nobukti = '%' + req.body.cari + '%';
    var tgl_awal = req.body.tglawal;
    var tgl_akhir = req.body.tglakhir;
    var periode = req.body.periode;
    connection.query("SELECT * FROM beli WHERE IF(?<>'',NO_BUKTI LIKE ?,TRUE) AND TGL BETWEEN ? AND ? AND FLAG='BL' AND PER=? AND TYP='I' AND GOL='A'", [nobukti, nobukti, tgl_awal, tgl_akhir, periode], function (error, rows, fields) {
        if (error) {
            console.log(error);
        } else {
...
exports.editheaderbtbbahanimport = function (req, res) {
    var NO_BUKTI = req.body.NO_BUKTI;
    var TGL = req.body.TGL;
    var CURR = req.body.CURR;
    var CURRNM = req.body.CURRNM;
...
exports.modalbtbbahanimport = function (req, res) {
...
exports.caribtbbahanimport = function (req, res) {
    var filter_cari = '%' + req.body.cari + '%';
...
exports.hapusbtbbahanimport = function (req, res) {
    var no_bukti = req.body.no_bukti;
